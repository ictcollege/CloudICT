...
'.$message.'
            ...
            $result = $this->email
                ->from('cloud_ict_system@ict.edu.rs')   // Optional, an account where a human being reads.
                ->to('filip.jericho.radojkovic@gmail.com')
                ->subject($subject)
                ->message($body)
                ->send();
...
class Admin extends Backend_Controller {
    ...
    public function getAllUsers()
    {
        ...
        { 
            ...
            $data['data'][$i][] = $user["UserFullname"];
            $data['data'][$i][] = $user["UserEmail"];
            $diskquota = $user["UserDiskQuota"];
            $diskused = $user["UserDiskUsed"];
            $percentage =  round(($diskused/$diskquota) * 100);
            $data['data'][$i][] = $this->formatBytes($diskquota);
